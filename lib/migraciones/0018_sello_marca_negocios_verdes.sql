...
--
-- El Sello Marca de Negocios Verdes (respaldado por Marca País, ver 4.4.3
-- del PNNV 2022-2030 y la cobertura de prensa del Galardón Verde 2025 de
-- la CDMB) es un reconocimiento ADICIONAL e independiente de
-- nivel_desarrollo — un negocio puede ser verificado y tener el sello, o
-- ser negocio ancla y tener el sello, o cualquier combinación. No es un
-- valor más dentro de nivel_desarrollo (esos siguen siendo mutuamente
-- excluyentes: un negocio está en uno solo a la vez), por eso es una
-- columna booleana propia, no un cuarto nivel.
alter table negocios add column if not exists sello_marca boolean not null default false;

-- guardar_negocio gana un parámetro más: p_sello_marca boolean. Cambia el
-- tipo de parámetros (agrega uno), hay que borrar la firma anterior
-- explícitamente antes de crear la nueva o PostgREST queda con dos
-- versiones sobrecargadas sin poder elegir cuál usar — mismo motivo que
-- 0015 y 0016.
