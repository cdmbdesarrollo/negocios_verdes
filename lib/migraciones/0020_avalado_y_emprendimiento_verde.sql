...
--
-- Reemplaza el enum nivel_desarrollo (en_verificacion/verificado/
-- negocio_ancla) por dos columnas booleanas independientes, mismo patrón
-- que sello_marca (0018) y aval_confianza (0019):
--
-- - avalado: reconocimiento público ("Negocio avalado por la CDMB"),
--   reemplaza al viejo valor 'verificado'/'negocio_ancla' del enum. Se
--   muestra en la ficha pública, es filtrable en /buscar, y es
--   independiente de sello_marca/aval_confianza (un negocio puede tener
--   cualquier combinación de las 3).
-- - emprendimiento_verde: SOLO uso interno de CDMB — negocios no avalados
--   o en proceso. A propósito NO se muestra en la ficha pública ni es
--   filtrable en /buscar (pedido explícito): existe únicamente para que el
--   admin deje esa información en la base de datos.
--
-- Backfill antes de borrar la columna vieja: 'verificado'/'negocio_ancla'
-- pasan a avalado=true (ya eran un reconocimiento otorgado por CDMB bajo
-- el sistema viejo); 'en_verificacion' (el default de siempre) pasa a
-- emprendimiento_verde=true, igual que quedaría un negocio nuevo sin tocar
-- estos campos.
...
update negocios set avalado = true
  where nivel_desarrollo in ('verificado', 'negocio_ancla');
update negocios set emprendimiento_verde = true
  where nivel_desarrollo = 'en_verificacion';

alter table negocios drop column if exists nivel_desarrollo;

-- guardar_negocio cambia p_nivel_desarrollo por p_avalado +
-- p_emprendimiento_verde — hay que borrar la firma anterior explícita
-- antes de crear la nueva, mismo motivo que 0015/0016/0018/0019.
drop function if exists guardar_negocio(
  uuid, text, uuid[], text, text, double precision, double precision,
  text, text, text, text, text, text, text, text, text, text, text,
  boolean, boolean, uuid[], uuid[], boolean, boolean
);
...
begin
  ...
  if not v_ya_existe then
    ...
    insert into negocios (
      ...
      foto_portada_url, foto_portada_path, destacado, activo,
      sello_marca, aval_confianza, avalado, emprendimiento_verde, created_by
    ) values (
      ...
      p_foto_portada_url, p_foto_portada_path, p_destacado, p_activo,
      p_sello_marca, p_aval_confianza, p_avalado, p_emprendimiento_verde, auth.uid()
    );
  else
